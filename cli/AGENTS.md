...
| File | Description |
|------|-------------|
| `package.json` | Package manifest: `bin` `9router` → `./cli.js`; scripts `dev` (nodemon), `build` (build-cli), `pack:cli`, `publish:cli`, `postinstall`; version independent of root (currently `0.5.66`) |
| `cli.js` | CLI entry: arg parsing (`--port/-p`, `--host/-H`, `--no-browser/-n`, `--log/-l`, `--tray/-t`, `--skip-update`, `--help`, `--version`), update check, process/port cleanup, server spawn + crash-restart, interface menu, tray init. `xai video` subcommand (cli.js:70-72) early-exits before the launcher flow |
...
| Directory | Purpose |
|-----------|---------|
| `hooks/` | Postinstall + runtime self-heal: preinstalls `sql.js` / `better-sqlite3` / `systray2` into `~/.9router/runtime` (no deeper AGENTS.md yet) |
| `scripts/` | `build-cli.js` (builds root app with `NEXT_DIST_DIR=.next-cli-build` + `NEXT_TRACING_ROOT_MODE=workspace`, copies to `cli/app/`, runs `buildMitm.js` to bundle `cli/app/src/mitm/bundle.js`, syncs version into root `package.json`), `buildMitm.js` (no deeper AGENTS.md yet) |
| `src/` | CLI source: terminal UI, API client, menus, tray, utils (see `src/cli/AGENTS.md`) |
...
- Server spawn uses node flags `--dns-result-order=ipv4first --max-old-space-size=6144` (cli.js:615) — preserve when touching spawn logic.
...
- There is no test suite in `cli/`. Root `tests/` does not cover the CLI.
...
- All dashboard state access from the CLI goes through `src/cli/api/client.js` (HTTP against the running gateway, token header `x-9r-cli-token` derived from the `machine-id` file via `MACHINE_ID_FILE`). Menus in `menus/` call this client; shared prompt/display/menu helpers live in `utils/`.
...
- The running server's state lives under `~/.9router` — see `hooks/sqliteRuntime.js` `getDataDir()`. CLI runtime deps land in `~/.9router/runtime/node_modules`.
