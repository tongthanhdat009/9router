...
- Affinity diagnostics JSONL (`affinity.jsonl`, gated by `ENABLE_AFFINITY_LOG=1` or `enableObservability`) — events `affinity.request`, `affinity.rebind`, `affinity.limit_expired`, `affinity.throughput.*`

## Session Affinity

Session affinity pins a client session (derived via `open-sse/utils/sessionManager.js` from `x-session-id`/`x-mux-workspace-id`/`prompt_cache_key` etc.) to a selected route (combo model) and provider account (`src/sse/services/sessionAffinity.js`). Stores are process-local `Map`s with 30-min sliding TTL (`routes` 4096, `accounts` 8192). Lookup is `getRouteAffinity(sessionId, routeScope)` / `getAccountAffinity(sessionId, provider, model)` in `src/sse/handlers/chat.js` before combo/account selection; retained only on successful (`response.ok`) logical requests via `bindRouteAffinity`/`bindAccountAffinity` with `requestCount`.

- **Request-count limit:** `AFFINITY_MAX_LOGICAL_REQUESTS` (env, `0`=disabled, positive int via `positiveEnv`, suggested `30`) caps consecutive successful logical requests per `(session, routeScope)` and per `(session, provider, model)` binding. Checked before selection (limit takes precedence over throughput escape); on `requestCount >= MAX` the binding is invalidated (`affinity.limit_expired`) and the request is treated as affinity miss. Increment is exactly once per successful outer logical request (`bind*` with `{requestCount}`); failures/aborts and inner retries (executor retry, `while(true)` account loop, combo per-model loop) do not increment. Applied in both `handleChat` outer combo path and `handleSingleModelChat` inner combo path, and in the account `while(true)` loop only on the outer attempt (`isOuterAccountAttempt`).
