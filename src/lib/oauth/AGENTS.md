...
Per-provider OAuth flows for 9Router — the definitions powering `src/app/api/oauth/*` (browser login, callback, token exchange) and `open-sse` token refresh. One provider file per upstream (claude, codex, cursor, gemini-cli, github, grok-cli, kiro, xai, muse, zcode, freebuff, …), plus services that orchestrate exchange/refresh and shared PKCE/IDP utilities. Provider definitions are consumed by `src/lib/oauth/services/` and the app's OAuth API routes; the resulting credentials feed `src/lib/db/repos/connectionsRepo.js`.

## Key Files

| File | Description |
|------|-------------|
| `providers.js` | Re-export of `./providers/index.js` (the registry) |
...
| `providers/` | 25 provider flow files + `_shared.js`: antigravity, claude, cline, clinepass, codebuddy-cn, codebuddy-intl, codex, cursor, freebuff, gemini-cli, github, gitlab, grok-cli, iflow, kilocode, kimchi, kimi, kiro, muse, qoder, trae, windsurf, xai, zcode, zed |
| `services/` | 15 service modules + `index.js`: antigravity, claude, codex, cursor, gemini, github, iflow, kimchi, kiro, muse, openai, qoder, xai, zcode + `oauth.js` (generic `OAuthService`) |
...
- One provider = one flow file in `providers/`. Add a new provider by adding a file there and registering it in `providers/index.js`; add a service in `services/` when exchange/refresh needs orchestration beyond the generic `OAuthService`.
...
- `unit/xai-oauth-service.test.js` times out (5s) when the xAI endpoint-discovery fetch is not reachable/mocked — expected red on plain checkout.
