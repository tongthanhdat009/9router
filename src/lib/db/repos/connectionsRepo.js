...
const OPTIONAL_FIELDS = [
  ...
  "consecutiveUseCount", "idToken", "lastRefreshAt", "lastErrorType", "reauthRequiredAt",
];
...
export async function createProviderConnection(data) {
  ...
  db.transaction(() => {
    ...
    if (existing) {
      // Local error-surface reset first (oauth active revalidation), then the
      // upstream health-state clear (locks/backoff/rate-limit). Both are
      // testStatus-gated, so an explicit active validation wipes stale state.
      const localReset = { ...data };
      if (data.authType === "oauth" && data.testStatus === "active") {
        localReset.lastErrorType = null;
        localReset.reauthRequiredAt = null;
      }
      const normalized = resetHealthStateOnActivation(existing, localReset);
