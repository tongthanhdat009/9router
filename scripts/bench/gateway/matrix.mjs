...
import {small, identity, verifyRequests, requireClean} from './contracts.mjs';
export function cellsFor(mode) {
  ...
  else if(mode === 'threshold') for(const threshold of [128,256,512].map(n=>n*1024)) for(const delta of [-1,0,1]) add(threshold+'/'+delta,8,{TRAFFIC_PACING_THRESHOLD:String(threshold)},{bytes:threshold+delta});
  else if(mode === 'mixed') for(const p of ['off','fixed','bytes:8:5:15']) add(p,8,{TRAFFIC_PACER:p},{mixed:true});
...
export async function run(mode=process.env.MODE || 'spacing') {
  ...
  try {
    ...
    for(let rep=0;rep<repetitions;rep++) for(const cell of H.shuffled(cellsFor(mode),random)) for(const runtime of H.shuffled(runtimes,random)) {
      ...
      const g=await H.launchGateway(h,{runtime,extraEnv:cell.env,trust:cert.trustEnv});
      ...
      const heavyBodies=Array.from({length:cell.n},(_,i)=>cell.mixed?sizedMessages([64,192,300,512,1024,1700,300,64][i]*1024):messages);
      const expected={hp:[],ht:[],vp:[],vt:[]};
      let active=0,t0;
      const request=async (name,body,kind,scheduledOffsetMs=0) => {
        const offsetMs=performance.now()-t0, exposure=active;
        const s={kind,scheduledOffsetMs,offsetMs,schedulingDelayMs:offsetMs-scheduledOffsetMs,heaviesInFlightAtDispatch:exposure};
      ...
      try {
        ...
        if(cell.sustained) {
          ...
          await Promise.all(Array.from({length:8},async()=>{while(performance.now()<deadline){const i=index++;await request(i%2?'proxy':hn,i%5?small:messages,i%5?'small':'heavy',performance.now()-t0);}}));
          ...
          const tasks=heavyBodies.map(body=>request(hn,body,'heavy',0));
        ...
        assert.equal(row.telemetry.dropped,0,'telemetry overflow');
        row.verification={};
