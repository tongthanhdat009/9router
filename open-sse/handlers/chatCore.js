...
export function stripContinuityFields(body) {
  ...
  for (const msg of body.messages) {
    if (msg && typeof msg === "object") {
      if (Object.hasOwn(msg, "encrypted_content")) delete msg.encrypted_content;
      if (Object.hasOwn(msg, "reasoning_encrypted_content")) delete msg.reasoning_encrypted_content;
    }
  }
  return body;
}

export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, codexUpstreamWebsocket, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking, affinity, affinityDiagnostics, finalizeAffinityRequest, onRouteAffinityStreamComplete, requestId }) {
  ...
  const streamController = createStreamController({
    onDisconnect: (reason) => {
      ...
      finalizeAffinityRequest?.({ status: 499, usage: null });
      if (onDisconnect) onDisconnect(reason);
    },
    onError: () => { trackPendingRequest(model, provider, connectionId, false); finalizeAffinityRequest?.({ status: 502, usage: null }); },
  ...
  try {
    const result = await executor.execute({ model, body: translatedBody, stream, credentials, signal: streamController.signal, log, proxyOptions, requestId, codexUpstreamWebsocket });
    ...
    try {
      ...
      if (isUnrecoverableRefreshError(newCredentials)) {
        ...
        try {
          ...
          retryResult = await executor.execute({ model, body: translatedBody, stream, credentials, signal: streamController.signal, log, proxyOptions, requestId, codexUpstreamWebsocket });
  ...
  const sharedCtx = { provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, pxpipe: pxpipeSummary, reqTag, log, affinity, affinityDiagnostics, finalizeAffinityRequest, onRouteAffinityStreamComplete };
