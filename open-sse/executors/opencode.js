...
export class OpenCodeExecutor extends BaseExecutor {
  ...
  deriveRequestContext(body, credentials, _request = {}, rawBody = body) {
    const sessionId = resolveOpencodeSession(rawBody, credentials);
    return { sessionId, requestId: resolveOpencodeRequestId(rawBody, credentials, sessionId) };
  }
  ...
  buildHeaders(credentials, stream = true, url = "", _model = null, ctx = {}) {
    const raw = credentials?.rawHeaders || {};
    const lower = {};
    ...
    const session = ctx.sessionId || resolveOpencodeSession(null, credentials);
    const requestId = ctx.requestId || resolveOpencodeRequestId(null, credentials, session);
