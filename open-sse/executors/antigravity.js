...
import { cleanJSONSchemaForAntigravity, isAntigravitySchemaClean, normalizeGeminiContents } from "../translator/formats/gemini.js";
  ...
  transformRequest(model, body, stream, credentials) {
    ...
    const rawContents = (body.request?.contents || []).map(c => {
      ...
      const modifiedParts = parts?.map(p => {
        ...
        if (callSig) return { ...p, thoughtSignature: callSig };
        return p;
      });
