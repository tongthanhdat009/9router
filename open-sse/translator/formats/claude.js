...
export function anchorClaudeCache(body) {
  if (!body || typeof body !== "object") return body;
  // Strip markers Anthropic rejects outright (deferred tools cannot carry
  // cache_control, #3567), then preserve any remaining caller markers instead
  // of re-anchoring — satisfies both marker-preserve and deferred-strip suites.
  if (Array.isArray(body.tools)) {
    for (const tool of body.tools) {
      if (tool?.defer_loading === true) delete tool.cache_control;
    }
  }
  if (hasExplicitCacheControl(body)) return body;
...
export function prepareClaudeRequest(body, provider = null, apiKey = null, connectionId = null, rawHeaders = null, sessionId = null) {
  ...
  if (body.tools && Array.isArray(body.tools)) {
    ...
    const lastCacheable = preserveCacheControl ? -1 : lastCacheableToolIndex(body.tools);
